...
{
	n=7; s=0; t=n-1;
	memset(f,0,sizeof(f));			//初始化一个零流
	memset(c,INF,sizeof(c));		//建立c
	for (int i=0;i<n;i++)
		c[i][i]=0;
...
void disp(int a[MAXV][MAXV])		//输出一个矩阵
{
	int i,j;
	for (i=0;i<n;i++)
	{
...
/*
	...
	n=6; s=0; t=n-1;
	int f[MAXV][MAXV];
	memset(f,0,sizeof(f));			//置一个零网络流
	int c[MAXV][MAXV]={				//一个网络流容量
		{0,4,5,INF,INF,INF},
		{INF,0,1,3,INF,INF},
		{INF,INF,0,INF,2,INF},
		{INF,INF,INF,0,INF,5},
