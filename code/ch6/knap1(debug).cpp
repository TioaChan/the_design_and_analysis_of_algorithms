...
void dispnode(NodeType e)		//输出一个队列结点
{
	printf("e.no=%d,e.i=%d,e.w=%d,e.v=%d, e.ub=%g, x:[",e.no,e.i,e.w,e.v,e.ub);
	for (int i=1;i<=n;i++)
		printf("%d ",e.x[i]);
	printf("]\n");
}
...
bool EnQueue(NodeType e,priority_queue<NodeType> &qu)	//结点e进队qu
{
	if (e.i==n)					//到达叶子结点
	{
